...
class PerformanceServiceImpl implements PerformanceService {
    ...
    public Interview save(Interview interview) {
        try {
            return performanceRepository.save(interview);
        }
        catch (DataAccessException e) {
            throw new RuntimeException(e.getMostSpecificCause().getMessage());
        }
    ...
    private Interview findById(Long id) {
        return performanceRepository.findById(id).orElseThrow(()-> new ResourceNotFoundException("Interview with id: " + id + " not found"));
