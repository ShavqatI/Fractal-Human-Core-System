...
import com.fractal.domain.employee_management.business_trip.expense.resource.BusinessTripExpenseResourceService;
...
@RestController
@RequestMapping("/api/v1/employee-management/business-trip/expense/{expenseId}/resource")
@RequiredArgsConstructor
public class BusinessTripExpenseResourceController {

    private final BusinessTripExpenseResourceService resourceService;
    ...
    public ResponseEntity<List<ResourceResponse>> getAll(@PathVariable Long expenseId) {
        return ResponseEntity.ok(resourceService.getAllByBusinessTripExpenseId(expenseId).stream().map(resourceService::toDTO).collect(Collectors.toList()));
    }
    @GetMapping("/{id}")
